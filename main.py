...
def main():
    ...
    player_name = input("Как тебя будут звать на здесь, на Просторах? ")
    ...
    while True:
        ...
        print(f"{player.name} Здоровье: {player.hp}, Мана: {player.mp}")
        ...
        action = input("Выберите действие: (w/a/s/d для перемещения, f для атаки, i для интеракции, q для выхода, локации): ").lower()
                ...
                for building_x, building_y, description in current_location.buildings:
                    if (building_x, building_y) == player_pos:
                        ...
                        if description == "Маленький домик":
                            print("Ты вошёл в небольшой домик. Пустовато.")
                        elif description == "Ратуша":
                            print("Ты посетил ратушу. Ты чувствуешь, что готов закончить начатое.")
                            ...
                            print("Ты подходишь к большому дому. Он принадлежит одному из знатных родов. На входе табличка на неизвестном тебе языке.")
                        elif description == "Твой дом":
                            print("Ты зашёл в свой дом. Уют пронзил твоё тело. Но нельзя останавливаться надолго.")
        ...
        if action in ('w', 'a', 's', 'd'):
            ...
            if not check_collision(new_pos, current_location.grid):
                ...
                if player.hp <= 0:
                    print("Ты умер! Позорная смерть, однако.")
                    break
        ...
        if all_quests_complete and current_location_name == "хаб" and player_pos == (2, 3): # Проверка, если игрок находится внутри Ратуши.
              print("Ты завершил все задания и завершил игру! Поздравляем!!!")
              break
